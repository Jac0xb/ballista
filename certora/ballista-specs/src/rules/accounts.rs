...
use super::util::{
    constrained_program, pick, pinned_program, spec_program, unset_registers, PINNED_ADDRESS,
    PINNED_OWNER, REGISTERS,
};
...
pub fn rule_pinned_address_and_owner_are_enforced() {
    ...
    let views = nondet_account_views::<1, 0>();
    let account = &views[0];
    let pinned = account.address().as_ref() == PINNED_ADDRESS
        && account.owner().as_ref() == PINNED_OWNER;
    match validate_runtime_accounts(&program, &views[..]) {
