    ...
    U: InstructionData,
{
    pub async fn to_transaction(
        ...
        // dedupe signers
        let mut deduped_signers: Vec<&Keypair> = vec![];
        for signer in self.signers.iter() {
            let mut has_duplicate = false;
            for deduped_signer in deduped_signers.iter() {
                if deduped_signer.pubkey() == signer.pubkey() {
                    has_duplicate = true;
                }
            }

            if !has_duplicate {
                deduped_signers.push(signer);
            }
        }

        let tx = VersionedTransaction::try_new(
            VersionedMessage::V0(
                v0::Message::try_compile(&self.payer, &ixs, &[], recent_blockhash).unwrap(),
            ),
            &deduped_signers,
        )
        .unwrap();
        ...
        Self: OnSuccessfulTxExec,
    {
        ...
        // ::new_with_payer(&ixs, Some(&self.payer));
