...
// Instanciate Kinobi.
const kinobi = k.createFromIdl(path.join(programDir, 'ballista.json'));
...
kinobi.update(
  k.addPdasVisitor({
    ballista: [
      k.pdaNode('schema', [
        k.constantPdaSeedNodeFromString('utf8', 'schema'),
        k.variablePdaSeedNode('user', k.publicKeyTypeNode()),
        k.variablePdaSeedNode('id', k.numberTypeNode('u8')),
      ]),
    ],
  }),
);
